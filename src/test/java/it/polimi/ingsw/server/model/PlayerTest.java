...
import it.polimi.ingsw.server.model.decks.cards.ResourceObjectiveCard;
...
class PlayerTest {
    ...
    void isValidMove() throws InvalidMoveException {
        ...
        assertEquals(SC_002, player.getPlayArea()[41][41]);

        player.initializeHand(new ArrayList<>());
        player.drawCard(playableCard3);
        player.drawCard(playableCard4);

        assertTrue(player.getHand().contains(playableCard3));
        assertTrue(player.getHand().contains(playableCard4));

        //gioco la carta PC_002 in una posizione già occupata e verifico che venga lanciata l'eccezione
        assertTrue(player.getPlayArea()[41][41] == SC_002);
        ...
        assertEquals(playableCard4, player.getPlayArea()[42][40] );
    }
    ...
    @Test
    public void drawWithNullHand() {
        // Assicurati che la mano sia null
        player.getHand().clear();

        // Pesca una carta
        player.drawCard(PC_001);

        // Verifica che la mano non sia più null
        assertNotNull(player.getHand(), "La mano non dovrebbe essere null dopo aver pescato una carta.");

        // Verifica che la carta sia stata aggiunta correttamente alla mano
        assertEquals(1, player.getHand().size(), "La mano dovrebbe contenere una carta.");
        assertTrue(player.getHand().contains(PC_001), "La carta pescata dovrebbe essere presente nella mano.");
    }
    @Test
    void objectiveScoreCalculator() {
        ...
        stock.put(Resource.PLANT_KINGDOM, 5);
        stock.put(Resource.INSECT_KINGDOM, 3);
        stock.put(Resource.FUNGI_KINGDOM, 2);
        stock.put(Resource.ANIMAL_KINGDOM, 4);
        stock.put(SpecialObject.MANUSCRIPT, 0);
        stock.put(SpecialObject.INKWELL, 0);
        stock.put(SpecialObject.QUILL, 1);
        ...
        player.drawCard(GC_051);
        player.placeCard(GC_051, new Coordinates(40, 40));
    ...
    /**
     * Tests if a player has enough resource in his stock to play a gold card
     */
    @Test
    void hasEnoughStock() {
        PlayableCard playableCard = PC_007;

        // Initialize player's stock
        Map<CornerContent, Integer> stock = new HashMap<>();
        ...
        //caso in cui lancia un' eccezione
        assertThrows(InvalidMoveException.class, () -> {
            player.hasEnoughStock(GC_051);
        });

        stock.put(Resource.PLANT_KINGDOM, 2);
        stock.put(Resource.INSECT_KINGDOM, 1);
        stock.put(Resource.FUNGI_KINGDOM, 0);
        stock.put(Resource.ANIMAL_KINGDOM, 5);
        stock.put(SpecialObject.MANUSCRIPT, 5);
        stock.put(SpecialObject.INKWELL, 0);
        stock.put(SpecialObject.QUILL, 0);
        player.setStock(stock);

        // Verifica che non venga lanciata alcuna eccezione
        assertDoesNotThrow(() -> {
            player.hasEnoughStock(GC_051);
        });
    };
